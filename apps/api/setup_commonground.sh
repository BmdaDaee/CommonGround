...
// services/session.js
// Unified entrypoint for sending a message to Bently

const { askPersona, MODES } = require("./ai");
const { getUserById, getRelationshipById } = require("./db");

async function sendToBently({ userId, relationshipId, message, mode }) {
  if (!userId) {
    throw new Error("sendToBently requires userId");
  }
  if (!message || typeof message !== "string") {
    throw new Error("sendToBently requires a non-empty message string");
  }
